...
for (i1 = 0; i1 <= n; ++i1)
  {if (i1 == n){j1 = 0;}else{j1 = 1;}
	...
	//for (int i=1;i<=37;++i){unitrand();}
	//vec = (2*conv(unitrand())-1)*0.8*(cos(theta),sin(theta));
	...
	if (i1==2 && i2==2)
	  {
		...
		draw(x-(0.8,0)-0.3*vec..x-(0.8,0)+0.6*vec,pn1,Arrow(30,30*(100-i)/100.,Fill));
	      }
	    draw("$t$",x..x-(0.5,0.5)..x-(1,0),linewidth(4)+fontsize(40),Arrow(30,Fill));
	    label("$U$",x-(1.2,-0.2),fontsize(40));
	  }
	else
	  {
	    for (int i=1;i<=100;++i)
