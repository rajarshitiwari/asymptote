...
surface X = scale3(0.05)*unitsphere;
surface Y = scale3(0.08)*unitsphere;
surface Z = scale3(0.08)*unitsphere;
  ...
  {if (i1 == n){j1 = 0;}else{j1 = 1;}
	...
	for (int i3 = 0; i3 <= n; ++i3)
	  {if (i3 == n){j3 = 0;}else{j3 = 1;}
	    p1 = (i1,i2,i3);
	    p2 = p1 + (0.5,0.5,0.5);
	    draw(shift(p1)*X,green+linewidth(7.0));
	    // if((i1+i2+i3)%2 ==1)
	    //   {
	    // 	if (j1*j2*j3 !=0) draw(shift(p2)*Y,rgb(1,0,0)+linewidth(4.0));
	    //   }
	    // else
	    //   {
	    // 	if (j1*j2*j3 !=0) draw(shift(p2)*Z,rgb(0,0,1)+linewidth(4.0));
	    //   }
	    pth1=p1--p1+(j1,0,0);
	    pth2=p1--p1+(0,j2,0);
	    pth3=p1--p1+(0,0,j3);
	    ...
	    // if (j1*j2*j3 !=0)
	    //   {
	    // 	pth1=p2-0.5*(j1,0,0)--p2+0.5*(j1,0,0);
	    // 	pth2=p2-0.5*(0,j2,0)--p2+0.5*(0,j2,0);
	    // 	pth3=p2-0.5*(0,0,j3)--p2+0.5*(0,0,j3);
	    // 	draw(pth1,gray(0.9)+thick2,lit);
	    // 	draw(pth2,gray(0.9)+thick2,lit);
	    // 	draw(pth3,gray(0.9)+thick2,lit);
	    //   }
  ...
  {if (i1 == n){j1 = 0;}else{j1 = 1;}
	...
	for (int i3 = 0; i3 <= n; ++i3)
	  {if (i3 == n){j3 = 0;}else{j3 = 1;}
	    ...
	    if((i1+i2+i3)%2 ==1)
	      {
		draw(shift(p2)*Y,rgb(1,0,0)+linewidth(4.0));
		pth1=p2--p2+(1,1,0);
		pth2=p2--p2+(1,0,1);
		pth3=p2--p2+(0,1,1);
		draw(pth1,red+thick1,lit);
		draw(pth2,red+thick1,lit);
		draw(pth3,red+thick1,lit);
	      }
	    else
	      {
		draw(shift(p2)*Z,rgb(0,0,1)+linewidth(4.0));
	      }
	    pth1=p2--p2+(j1,0,0);
	    pth2=p2--p2+(0,j2,0);
	    pth3=p2--p2+(0,0,j3);
