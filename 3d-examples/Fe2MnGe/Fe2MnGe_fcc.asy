...
triple A1=(0,1,1),A2=(1,0,1),A3=(1,1,0);

n=1;n1 = n;n2 = 2*n; n3=2*n;
      ...
      { j2 = (i2==n2) ? 0 : 1; k2 = (i2==0) ? 0 : 1;
        for (int i3 = 0; i3 <= n3; ++i3)
          { j3 = (i3==n3) ? 0 : 1; k3 = (i3==0) ? 0 : 1;
            x = (i1*A1 + i2*A2 + i3*A3);
            x1 = x; x2 = x + (1,1,1); x3 = x + 0.5*(1,1,1); x4 = x + 0.5*(3,3,3);
            //if ((i1+i2+i3)%2==1)
            //{
                //draw(shift(x)*Ge,gray(0.7)+linewidth(7.0));
            //} else
            //{
            draw(shift(x1)*Mn,red+linewidth(4.0));
            draw(shift(x2)*Mn,blue+linewidth(4.0));
            draw(shift(x3)*Mn,green+linewidth(4.0));
            draw(shift(x4)*Mn,cyan+linewidth(4.0));
            ...
            pth1=x1..x1+(1,0,0); pth2=x1..x1+(0,1,0); pth3=x1..x1+(0,0,1);
            draw(pth1,gray(0.5)+thick1,lit); draw(pth2,gray(0.5)+thick1,lit); draw(pth3,gray(0.5)+thick1,lit);
            pth1=x1..x1-(1,0,0); pth2=x1..x1-(0,1,0); pth3=x1..x1-(0,0,1);
            draw(pth1,gray(0.5)+thick1,lit); draw(pth2,gray(0.5)+thick1,lit); draw(pth3,gray(0.5)+thick1,lit);

            //pth1=x3..x3+(1,0,0); pth2=x3..x3+(0,1,0); pth3=x3..x3+(0,0,1);
            //draw(pth1,gray(0.5)+thick1,lit); draw(pth2,gray(0.5)+thick1,lit); draw(pth3,gray(0.5)+thick1,lit);
            //pth1=x3..x3-(1,0,0); pth2=x3..x3-(0,1,0); pth3=x3..x3-(0,0,1);
            //draw(pth1,gray(0.5)+thick1,lit); draw(pth2,gray(0.5)+thick1,lit); draw(pth3,gray(0.5)+thick1,lit);

            // bcc Mn-Fe hopping //
            pth1=x1..x1+0.5*(1,1,1); pth2=x1..x1-0.5*(1,1,1); // f,F
            draw(pth1,gray(0.5)+thick1,lit); draw(pth2,gray(0.5)+thick1,lit);
            pth1=x1..x1+0.5*(1,-1,-1); pth2=x1..x1-0.5*(1,-1,-1); // f,F
            draw(pth1,gray(0.5)+thick1,lit); draw(pth2,gray(0.5)+thick1,lit);
            pth1=x1..x1+0.5*(-1,-1,1); pth2=x1..x1-0.5*(-1,-1,1); // f,F
            draw(pth1,gray(0.5)+thick1,lit); draw(pth2,gray(0.5)+thick1,lit);
            pth1=x1..x1+0.5*(-1,1,-1); pth2=x1..x1-0.5*(-1,1,-1);
            draw(pth1,gray(0.5)+thick1,lit); draw(pth2,gray(0.5)+thick1,lit);
...
draw((0,0,0)..(2*n,0,0),linewidth(3)+red,Arrow3(30,10));
draw((0,0,0)..(0,2*n,0),linewidth(3)+green,Arrow3(30,10));
draw((0,0,0)..(0,0,2*n),linewidth(3)+blue,Arrow3(30,10));
bbox(rgb(0,0,0),Fill);
