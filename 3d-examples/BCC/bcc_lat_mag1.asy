...
currentprojection = perspective(camera=(5.798469868556706,-6.978648421643896,3.3186440574564076),
				up=(-0.000621564920217976,0.0012376703988248106,0.005721741135573029),
				target=(1.161049,1.164062,1.05352),
				zoom=1,
				angle=23.58678739439001,
				autoadjust=false);
  ...
  {if (i1 == n){j1 = 0;}else{j1 = 1;}
	...
	for (int i3 = 0; i3 <= n; ++i3)
	  {if (i3 == n){j3 = 0;}else{j3 = 1;}
	    ...
	    draw(shift(xi + b1)*Fe, orange+linewidth(1.0));
		...
		vec=expi(0, 0); scolor = rgb(1.0,0.0,0.0)+linewidth(5);
	      ...
	      {
	    	vec=expi(pi, 0); scolor = rgb(0.0,1.0,0.0)+linewidth(5);
	      }
	    draw(xi+b1-0.25*vec..xi+b1+0.3*vec, scolor, Arrow3(30), lit);
  ...
  {if (i1 == n){j1 = 0;}else{j1 = 1;}
	...
	for (int i3 = 0; i3 <= n; ++i3)
	  {if (i3 == n){j3 = 0;}else{j3 = 1;}
	    ...
	    draw(shift(xi + b2)*Mo, orange+linewidth(1.0));
	    if ((i1+i2+i3)%2 == 0)
	      {
		vec=expi(0, 0); scolor = rgb(1.0,0.0,0.0)+linewidth(5);
	      }
	    ...
	    draw(xi+b2-0.25*vec..xi+b2+0.3*vec, scolor, Arrow3(30),lit);
