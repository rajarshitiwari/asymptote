  ...
  {if (i1 == n) {j1 = 0;}else {j1 = 1;}
	...
	for (int i3 = imin; i3 <= n; ++i3)
	  {if (i3 == n) {j3 = 0;}else {j3 = 1;}
	    ...
	    if((i1+i2+i3)%2 ==0)
	      {
		draw(shift(vec1)*Zinc,rgb(0,0.8,0)+linewidth(4.0),lit); // Z
		pnb = gray(0.5)+thick3;
		// nearest neighbour bonds
		if (j1*j2*j3 == 1)
		  {
		    vec1 = vec + (1,1,1)/2.;draw(vec--vec1,pnb,lit);
		  }
		...
		if (j1*j2*j3 == 1)
		  {
		    vec1 = vec + (1,1,1)/2;
		    draw(shift(vec1)*Sulf,rgb(0.8,0,0)+linewidth(4.0),lit); // X
		  }

		if (k1*j2*j3 == 1)
		  {
		    vec2 = vec + (-1,1,1)/2;
		    draw(shift(vec2)*Sulf,rgb(0.8,0,0)+linewidth(4.0),lit); // X
		  }
